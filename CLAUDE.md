...
- A `PostToolUse` hook (`.claude/hooks/format-on-edit.mjs`) auto-runs rustfmt (`.rs`) and prettier (`.js`/`.css`/`.html`/`.json`/`.md`) after Edit/Write, so do not manually re-fix formatting issues that the hook already handles
