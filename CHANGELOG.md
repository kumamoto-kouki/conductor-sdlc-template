...
- 定常運用に**常時ロード総量の観測**を追加した（`full-sdlc.md`）。`CLAUDE.md` ＋ `.kiro/steering/` は毎セッション丸ごと読み込まれるが、棚卸し直後の 68,658 バイトから現在 81,457 バイトへ単調増加していることが、どこにも観測されていなかった。
...
- **`STATUS.md` は並行ブランチのマージで必ず衝突する。**`.gitattributes` は無く、解決手順の記述も無い。フックが有効なら「どちらか片方を採用」で雑に解決しても次のコミットで自己修復するが、フックが効かないクローンでは誤内容が残る。テンプレートの中核である並行開発で毎回起きる。
- **`.claude/settings.json` の `defaultMode` が `bypassPermissions`。** 公式ドキュメントは「隔離環境（コンテナ・VM）でのみ使え」と警告している。`deny` と `ask` は同モードでも強制されるため `git push` の人間承認は守られるが、`allow` リスト（65行）は同モードでは完全に無視される。自律性と安全性のトレードオフであり、PO の判断が要る。
- **3段階承認を記録する手段が `-y` しかない。** `approvals.*.approved: true` を書くのは `-y` 経路と `kiro-spec-batch`（無条件・抑止フラグ無し）だけで、`kiro-validate-design` は `allowed-tools` に Write が無く構造的に承認を記録できない。`STATUS.md` は PO に「要件を読んで承認する」と指示し続けるが、その行為に対応するコマンドが存在せず、停止時のエラーメッセージは `-y` を案内する。`README.md` が掲げる「各フェーズを人間が承認」の実装が欠けている。
- `.claude/skills/kiro-impl/SKILL.md` の手動モードが「subagent が使えなければメインコンテキストでレビューする」と自己レビューを許容しており、規律 C（Maker ≠ Checker）と正面から矛盾する。また `templates/reviewer-prompt.md` は「いかなるチェックも弱めていない」と宣言しながら、`kiro-review` の中核（承認済み事項の切り下げ検知・境界監査）を落としている。
