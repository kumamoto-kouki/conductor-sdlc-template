...
- **常時ロード総量の観測**：`cat CLAUDE.md .kiro/steering/*.md | wc -m` を実施し、**実施日と観測値は `.claude/maintenance.json` に記録する**（本文に数値を焼き込まない——焼き込んだ数値自体が陳腐化した実証がある）。予算超過は verify チェック7 が機械的に止める。増えていたら、条件付きでよい内容が `.claude/rules/` か playbook へ移せないかを検討する。
- **規律の撤去候補の提示**：独立レビューと振り返りの記録（`.kiro/specs/*/reviews/`・retrospective）には「今回引用した規律」が残る。**直近の点検周期で一度も発火（違反の検知・判断根拠としての引用）しなかった拘束は、撤去候補として PO へ提示する**。撤去の決定は従来どおり PO 承認が必須（事故由来の拘束を AI の判断だけで外さない）——この箇条は防波堤に「出口」を付けるものであり、防波堤を壊すものではない。
