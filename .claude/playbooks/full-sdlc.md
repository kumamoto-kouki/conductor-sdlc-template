...
- **常時ロード総量の観測**：`cat CLAUDE.md .kiro/steering/*.md | wc -c` を記録する。ここは毎セッション丸ごと読み込まれるため、増え続けると全セッションのコストと注意の希釈になる。削減量自体は目的にしないが、**総量が単調に増えていること自体に気づけないのが穴**なので数値を残す（実測: 棚卸し直後 68,658 → 2026-08-24 時点 81,913 バイト）。増えていたら、条件付きでよい内容が `.claude/rules/`（glob 一致時のみ読まれる）へ移せないかを検討する。
