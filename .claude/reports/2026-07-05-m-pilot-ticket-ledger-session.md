...
- **レビュアーの「派生先で verify を回す」が Maker の盲点を突いた**：初期状態同梱は Maker のビルド確認（grep）を通ったが、レビュアーが派生先で verify を実行し **0÷0=NaN%** の表示破損を発見。是正は derive.mjs のゼロ除算ガード（根本）＋名目値1（データ）の両建て
