...
3. **配役を確定**: `role-catalog.md` の標準キャスト（神谷=統括／遠藤・三浦=レーンリーダー／橘=運用／工藤・林・桜井=Maker／守屋・望月=Checker／安西=QA）から、今回動かす役を選ぶ。候補ロスターは必要フェーズで投入。
...
6. **可視化**: `docs/status-dashboard.html` を起点に進行を映す。**状態遷移トリガー**（着手→進行中→レビュー中→完了）で実施=神谷が全体整合、担当=橘が監視。
