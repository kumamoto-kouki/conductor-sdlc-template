...
> **テンプレ注記**: 本文中の事故記号（A2/A3 等）・具体例は、この手法を確立した実プロジェクトの事故／タスク例。あなたのプロジェクトでは**教訓（＝なぜ）だけ受け取り、記号や固有名は自分の実例に読み替える**。ペルソナ名（神谷・橘・遠藤 等）は**この手法の標準キャスト**としてそのまま使ってよい（`role-catalog.md`）。
...
- **マージ／撤去のタイミング**：状態遷移に揃える。
  ...
  - **橘が「滞留 worktree」を監視**：受理も破棄もされず残る worktree／merge 済みなのに撤去されないブランチ（例: 過去の `feat/機能D` 残置）を開示の健全性で拾う。
