...
| ---------------- | ------------------------ | -------------------------------------------------------------------- | --------------------------------------- |
| エンジニアリング | 🧑🏼‍💼 Engリーダー           | 実装エージェント群（実装BE・実装FE／`Agent` `isolation:"worktree"`） | 🛡️ EngRev（`kiro-review`・別実体）      |
| デザイン         | 🧑🏼‍🎨 Designリーダー        | デザイン実施者群（デザイン実装／Claude Design ＋ `/design-sync`）    | 🕵🏼‍♀️ デザインRev（独立 subagent・別実体） |
...
- **マージ／撤去のタイミング**：状態遷移に揃える。
  ...
  - **マージの場所／主体＝メイン作業ディレクトリ（統合ブランチをチェックアウト中）で統括が実施**。マージ直後に **worktree とブランチを撤去**（`git worktree remove` ＋ merge 済みブランチを `git branch -d`）。`pnpm test` の本数で二重カウントが無いことを確認。
...
モデル選択（コーディネーター/ワーカー分割・粒度の目安）は `CLAUDE.md` の Token / Cost Efficiency 節を参照する（正本はそちら、ここでは複製しない）。
...
- **サブエージェントの報告のうち、後続判断の重みを支える具体的事実主張は、検証コストが安ければ統括が独立に1点検証してから受理する**（全数検証はしない。判断基準は `CLAUDE.md` の Delegation tier 参照）。分解した計画の前提そのものが誤っている場合、下流の各タスクを検証しても前提の誤りは検出できない。
...
運用の実行状況の管理は**統括から分離**し、専任の運用マネージャー（運用）が担う（統括は委譲・受理に集中、運用健全性は別の目で監視＝職務分掌）。**詳細は `operations.md`（運用の正本）に分離**：報告系統・台帳化・チケット状態遷移とダッシュボード全体整合・WIP 上限・ID 体系・開示の健全性・相談窓口。運用は受理判断（証拠での合否）には踏み込まず「運用が健全に回っているか」を見る（運用 ≠ 統括の独立で統括の自己評価バイアスを避ける）。
...
- **コミット／ローカルマージはコンダクターが行う**。worker は自 `feat/<feature>` にコミットし、wave/マイルストーン境界でコンダクターがレビューして**統合ブランチ（現 `<統合ブランチ>`）へ merge**。コミットは意味単位、trailer に `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`。デフォルトブランチ上では先にブランチを切る。
