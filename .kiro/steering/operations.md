...
- **ダッシュボードは生成物・手編集しない**（2026-07-02 追加・PO）：`dashboard/status-dashboard.html` は `dashboard/status.json` ＋ `dashboard/dashboard-template.html` から `node scripts/generate-dashboard.mjs` がビルド時に生成する静的HTML（`docs/` はドキュメント専用に分離し、ダッシュボードのデータ・テンプレート・生成物一式は `dashboard/` に集約）。状態更新は `dashboard/status.json` のみを編集し、都度再生成する（ブラウザ実行時レンダリングは `file://` で JS 実行が制限される環境があり実際に発生したため撤去。生成物なら「手編集しない」規約も強制しやすく、git diff でレンダリング結果そのものを見れる）。**実施＝👨🏼‍💼 統括**（チケット移動と同一コミットで json 更新＋生成を行う）／**担当＝👩🏼‍💼 運用**（`status.json`＝実態になっているかを監視）。
